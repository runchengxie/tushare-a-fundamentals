...
* 该项目的默认参数的设计原则是默认下载过去10年，也就是40个季度的有效数据，其中，利润表、资产负债表、现金流量表、业绩预告，业绩快报，财务指标数据、主营业务构成、财报披露计划的回溯方法是从最近一个可下载数据的季度开始算起回溯40个季度，而分红送股则是把start对齐到当月1号，end用当月最后一天，逐月推进直到120个月

* 财务审计意见的下载需要逐个个股遍历，如采用10年的逻辑，下载时间约22小时，故`funda download --audit-only`的默认行为是下载x年，但是也可以通过`funda download --audit-only --year 5`等方式强制下载下载年份范围
