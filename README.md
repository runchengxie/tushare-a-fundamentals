...
> #### 备注：
> 
> 上述指令默认下载，检查，并导出近10年数据（即最近40个季度的季度累计合并报表数据，也就是tushare API的默认返回格式），但是必要时可通过附加选项来微调行为，例如只下载近五年的数据（--year 5），或者近下载某个股票代码的数据（--ticker 600036.SH）
...
1. Tushare的三张报表实际上提供了12种不同的类型，本项目走的是tushare的默认版本，也就是合并报表，但是可以通过修改根目录的config.yaml，或者临时加入指令选项来改变该项目的下载行为。
