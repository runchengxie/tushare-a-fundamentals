...
* 该项目的默认参数的设计原则是默认下载过去10年，也就是40个季度的有效数据，其中，利润表、资产负债表、现金流量表、业绩预告，业绩快报，财务指标数据，主营业务构成的回溯方法是从最近一个可下载数据的季度开始算起回溯40个季度，而分红送股则是把start对齐到当月1号，end用当月最后一天，逐月推进直到120个月
